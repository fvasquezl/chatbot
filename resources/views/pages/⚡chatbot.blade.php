...
new #[Layout('layouts.app', ['title' => 'Chatbot', 'fullHeight' => true])] class extends Component {
    ...
    {
        $conversation = DB::table('agent_conversations')->where('id', $id)->where('user_id', Auth::id())->first();
    ...
    {
        return DB::table('agent_conversations')->where('user_id', Auth::id())->orderByDesc('updated_at')->limit(50)->get()->map(fn($c) => ['id' => $c->id, 'title' => $c->title, 'updated_at' => $c->updated_at])->all();
    }
...
<div class="flex h-full overflow-hidden">
    ...
    <div class="flex min-w-0 flex-1 flex-col">
            ...
            <flux:dropdown>
                ...
                <flux:menu>
                    ...
                    @foreach ($this->conversations as $conv)
                        <flux:menu.item wire:key="mobile-conv-{{ $conv['id'] }}"
        ...
        {{-- Messages --}}
        <div class="min-h-0 flex-1 space-y-4 overflow-y-auto p-4 md:p-6" x-data
            ...
            @if (count($messages) === 0 && !$isProcessing)
                ...
                <div class="flex h-full flex-col items-center justify-center gap-6">
                    ...
                    <div class="text-center">
                        <h3 class="text-xl font-semibold text-zinc-900 dark:text-zinc-100">{{ __('Store Assistant') }}
                        </h3>
                        <p class="mt-1 text-lg text-zinc-500 dark:text-zinc-400">
                            {{ __('Ask me anything about products, orders, categories, or customers.') }}</p>
                    </div>

                    <div class="grid grid-cols-1 gap-2 sm:grid-cols-2">
                        @foreach ([__('What are the top selling products?'), __('How many orders are pending?'), __('Which products are low on stock?'), __('Show me revenue by order status')] as $suggestion)
                            <button wire:click="askSuggestion('{{ $suggestion }}')"
                                class="rounded-lg border border-zinc-200 px-4 py-2 text-left text-lg text-zinc-600 transition-colors hover:border-zinc-300 hover:bg-zinc-50 dark:border-zinc-700 dark:text-zinc-400 dark:hover:border-zinc-600 dark:hover:bg-zinc-800">
                                {{ $suggestion }}
                            </button>
                        @endforeach
                    </div>
            ...
            @else
                ...
                @if ($isProcessing)
                    <div class="flex justify-start">
                        <div
                            class="max-w-[80%] rounded-2xl bg-zinc-100 px-4 py-3 text-lg text-zinc-900 dark:bg-zinc-800 dark:text-zinc-100">
                            <div class="prose prose-lg dark:prose-invert max-w-none" wire:stream="response">
        ...
        <div class="shrink-0 border-t border-zinc-200 p-4 dark:border-zinc-700">
            <form wire:submit="sendMessage" class="flex items-center gap-2">
                <flux:input wire:model="message" placeholder="{{ __('Ask about products, orders, categories...') }}"
                    :disabled="$isProcessing" autofocus
                    class="flex-1 [&_input]:!text-lg [&_input]:sm:!text-lg [&_input]:!h-12" />

                <flux:button type="submit" variant="primary" icon="paper-airplane" :disabled="$isProcessing" class="!h-12 !w-12" />
            </form>
        </div>
    </div>
    ...
    <div
        class="hidden w-72 shrink-0 flex-col border-s border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900 lg:flex">
